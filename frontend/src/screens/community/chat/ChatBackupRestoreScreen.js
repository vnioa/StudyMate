    ...
    const fetchLastBackupInfo = async () => {
        try {
            ...
            if (response.data.lastBackup) {
                setLastBackupDate(new Date(response.data.lastBackup.date).toLocaleString());
            }
        } catch (error) {
            Alert.alert('오류', '백업 정보를 불러오는데 실패했습니다.');
        } finally {
            setLoading(false);
        }
    };
                ...
                { text: '취소', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        try {
                            ...
                            const response = await backupAPI.restoreFromBackup();
