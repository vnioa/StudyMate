...
const ProfileScreen = ({ navigation }) => {
    ...
    const [editType, setEditType] = useState(''); // 'nickname' 또는 'status'
    ...
    const handleEdit = (type) => {
        setEditType(type);
        setEditValue(type === 'nickname' ? userProfile.nickname : userProfile.statusMessage);
    ...
    const handleSave = async () => {
        try {
            ...
            if (response.data.success) {
                setUserProfile(prev => ({
                    ...prev,
                    [editType === 'nickname' ? 'nickname' : 'statusMessage']: editValue
                }));
            }
        ...
        >
            <View style={styles.modalContent}>
                <Text style={styles.modalTitle}>
                    {editType === 'nickname' ? '닉네임 수정' : '상태 메시지 수정'}
                </Text>
                <TextInput
                    style={styles.modalInput}
                    value={editValue}
                    onChangeText={setEditValue}
                    placeholder={editType === 'nickname' ? '닉네임을 입력하세요' : '상태 메시지를 입력하세요'}
                    maxLength={editType === 'nickname' ? 20 : 50}
                />
                <View style={styles.modalButtons}>
                    ...
                    <Pressable
                        ...
                        onPress={handleSave}
                    >
                        <Text style={[styles.modalButtonText, styles.saveButtonText]}>저장</Text>
                    </Pressable>
                </View>
            </View>
        </Modal>
    ...
    return (
        ...
        >
            ...
            <View style={styles.profileSection}>
                ...
                <Pressable onPress={() => handleEdit('nickname')}>
                    <Text style={styles.userName}>{userProfile.nickname || '닉네임을 입력하세요'}</Text>
                </Pressable>
                <Pressable onPress={() => handleEdit('status')}>
                    <Text style={styles.userEmail}>
                        {userProfile.statusMessage || '상태 메시지를 입력하세요'}
                    </Text>
                </Pressable>
                <View style={styles.statusButtons}>
                    <Pressable
                        style={[
                            styles.statusButton,
                            userProfile.activityStatus === '온라인' && styles.activeStatusButton
                        ]}
                        onPress={() => handleStatusChange('온라인')}
                    >
                    ...
                    <Pressable
                        style={[
                            styles.statusButton,
                            userProfile.activityStatus === '자리비움' && styles.activeStatusButton
                        ]}
                        onPress={() => handleStatusChange('자리비움')}
                    >
                        <View style={[styles.statusDot, { backgroundColor: theme.colors.warning }]} />
                        <Text style={styles.statusText}>자리비움</Text>
                    </Pressable>
                    <Pressable
                        style={[
                            styles.statusButton,
                            userProfile.activityStatus === '오프라인' && styles.activeStatusButton
                        ]}
                        onPress={() => handleStatusChange('오프라인')}
                    >
                        <View style={[styles.statusDot, { backgroundColor: theme.colors.inactive }]} />
